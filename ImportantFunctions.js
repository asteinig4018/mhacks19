...
let twitterData; // TO DO
let weatherData; // TO DO
let stockData; // TO DO
let approvalRatingData; // TO DO
let chartData; // TO-DO

xmlhttp.get("AllTweetData1.json", result => {
	twitterData = result;
	sliderCalculation();
})
xmlhttp.get("weather_us.json", result => {
	weatherData = result;
	sliderCalculation();
})
xmlhttp.get("sp500_price.json", result => {
	stockData = result;
	sliderCalculation();
})
xmlhttp.get("trump_approval_rating.json", result => {
	approvalRatingDataData = result;
	sliderCalculation();
})
